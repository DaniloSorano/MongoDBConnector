...
include "interface/createDbInterface.iol"
include "runtime.iol"
      ...
      if( is_defined( request )) {
        foreach ( node : request.docStructure ){
          ...
          /*containsRequest = updatePath;
            ...
            if( containsResponse == false ) {
              response = response + "
              for(index = 0, index < #" + tree.structurePath[0] + ", index++){}
                if( is_defined( request.documentUpdate." + path +  ") && request.documentUpdate." + path +  " != null) {
                  if( fieldBefore ) {
                    updateString = updateString + \", '" + tree.structurePath[0] + ".\" + index + \"." + tree.strcuturePath[1] + " ' : \" + request.documentUpdate." + path + " + \"\"
                  }else{
